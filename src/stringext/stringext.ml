      ...
      (let rec loop offset tokens =
         ...
         then [sub str offset ((length str) - offset)]
           ...
           (try
              ...
              if index = offset
              then "" :: (loop (offset + 1) (tokens + 1))
              else
                (let token = String.sub str offset (index - offset) in token
                   :: (loop (index + 1) (tokens + 1)))
            with | Not_found  -> [sub str offset ((length str) - offset)]) in
       loop 0 0)
let split_char_unbounded str ~on  =
  ...
  then []
    ...
    (let rec loop acc offset =
       ...
       with | Not_found  -> (sub str 0 (offset + 1)) :: acc in
     loop [] ((length str) - 1))
    ...
    (let sep = of_char on in
     let rec loop acc offset =
       try
         let index = rindex_from str offset on in
         if index = offset
         then loop (sep :: acc) (index - 1)
         else
           (let token = sub str (index + 1) (offset - index) in
            loop (sep :: token :: acc) (index - 1))
       with
       | Not_found  ->
           if offset >= 0 then (sub str 0 (offset + 1)) :: acc else acc in
     loop [] ((length str) - 1))
let split ?max  s ~on  =
  match max with
  | None  -> split_char_unbounded s ~on
  | Some max -> split_char_bounded s ~on ~max
let rindex_from_on s ~offset  ~on  =
  let rec loop i =
    if i < 0
    then raise Not_found
    else if String.contains on (s.[i]) then i else loop (i - 1) in
  loop offset
  ...
  let start_pos =
    ...
    let rec loop last_char i =
      ...
      then last_char
        ...
        if String.contains chars (s.[i])
        then loop (i + 1) (i + 1)
        else last_char in
    loop pos pos in
  let new_len = len - (start_pos - pos) in String.sub s start_pos new_len
    ...
    (let rec loop acc offset =
       try
         ...
         then loop ("" :: acc) (index - 1)
           ...
           (let token =
              trim_left_sub str ~pos:(index + 1) ~len:(offset - index)
                ~chars:trim in
            loop (token :: acc) (index - 1))
       with
       | Not_found  ->
           (trim_left_sub str ~pos:0 ~len:(offset + 1) ~chars:trim) :: acc in
     loop [] ((length str) - 1))
exception Found_int of int
let first_char_ne s c = ((String.length s) > 0) && ((s.[0]) <> c)
let trim_left s =
  if first_char_ne s ' '
  then s
  else
    (let len = String.length s in
     try
       for i = 0 to len - 1 do if (s.[i]) <> ' ' then raise (Found_int i)
       done;
       ""
     with | Found_int non_space -> sub s non_space (len - non_space))
...
let find_from ?(start= 0)  str ~pattern  =
  try
    ...
    None
  with | Found_int i -> Some i | _ -> None
let find_min l ~f  =
  let rec loop x fx =
    function
    | [] -> Some (x, fx)
    | x'::xs ->
        let fx' = f x' in if fx' < fx then loop x' fx' xs else loop x fx xs in
  match l with | [] -> None | x::xs -> loop x (f x) xs
let replace_all str ~pattern  ~with_  =
  let (slen,plen) = let open String in ((length str), (length pattern)) in
  let buf = Buffer.create slen in
  let rec loop i =
    match find_from ~start:i str ~pattern with
    | None  ->
        (Buffer.add_substring buf str i (slen - i); Buffer.contents buf)
    | Some j ->
    ...
    then Buffer.contents buf
      ...
      (let r =
           ...
           let e =
             find_min tbl
               ~f:(fun (pattern,with_)  ->
                     ...
                     | None  -> max_int
                     | Some j when j = i ->
                         raise (Found_replace (j, pattern, with_))
                     | Some j -> (found := true; j)) in
           match e with
           | None  -> None
           | Some ((pattern,with_),j) when !found -> Some (j, pattern, with_)
           | Some _ -> None
         with | Found_replace (j,pattern,with_) -> Some (j, pattern, with_) in
       match r with
       | None  ->
           (Buffer.add_substring buf str i (slen - i); Buffer.contents buf)
       | Some (j,pattern,with_) ->
           (Buffer.add_substring buf str i (j - i);
            Buffer.add_string buf with_;
            loop (j + (String.length pattern)))) in
  loop 0
let iteri f l =
  let rec loop i = function | [] -> () | x::xs -> (f i x; loop (succ i) xs) in
  loop 0 l
let of_list xs =
  let l = List.length xs in
  let s = Bytes.create l in
  iteri (fun i  -> fun c  -> Bytes.set s i c) xs; Bytes.unsafe_to_string s
let to_list s =
  let rec loop acc i =
    if i = (-1) then acc else loop ((s.[i]) :: acc) (pred i) in
  loop [] ((String.length s) - 1)
let of_array a =
  let len = Array.length a in
  let bytes = Bytes.create len in
  for i = 0 to len - 1 do Bytes.set bytes i (a.(i)) done;
  Bytes.unsafe_to_string bytes
let to_array s = Array.init (String.length s) (String.get s)
let cut s ~on  =
  let sep_max = (length on) - 1 in
  if sep_max < 0
  then invalid_arg "Stringext.cut: empty separator"
  else
       ...
       (let k = ref 0 in
        ...
        try
          while ((!i) + sep_max) <= s_max do
            if (unsafe_get s (!i)) <> (unsafe_get on 0)
            then incr i
            else
              (k := 1;
               while
...
let rcut s ~on  =
  let sep_max = (length on) - 1 in
  if sep_max < 0
  then invalid_arg "Stringext.rcut: empty separator"
  else
    (let s_max = (length s) - 1 in
     if s_max < 0
     then None
       ...
       (let k = ref 0 in
        ...
        try
          while (!i) >= sep_max do
            if (unsafe_get s (!i)) <> (unsafe_get on sep_max)
            then decr i
            else
              (let sep_start = (!i) - sep_max in
               ...
               while
                 ((!k) >= 0) &&
                   ((unsafe_get s (sep_start + (!k))) = (unsafe_get on (!k)))
                 do decr k done;
               if (!k) >= 0 then decr i else raise Exit)
            done;
          None
        with
        | Exit  ->
...
let chop_prefix s ~prefix  =
  ...
  then None
    ...
    (try
     ...
     with | _ -> None)
let drop s n =
  let l = String.length s in if n >= l then "" else String.sub s n (l - n)
